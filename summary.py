...
def main() -> None:
    ...
    def _pivot_parts(records: List[Dict[str, str]]):
        if pd:
            ...
            # 尝试解析小数
            dfp["产成率"] = dfp["产成率"].apply(_parse_number)
    ...
    def _pivot_value(records: List[Dict[str, str]]):
        if pd:
            ...
            for new_part, parts in DERIVED_PARTS.items():
                ...
                if sub.empty:
                    continue
                agg = sub.groupby("城市").agg({"含税金额": "sum", "销量": "sum"})
                agg["含税单价"] = agg.apply(
            ...
            combined = pd.concat([base, pd.DataFrame(derived_rows)], ignore_index=True)
