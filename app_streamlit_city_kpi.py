...
def _build_rate_lookup(part_records: List[Dict[str, str]]) -> Dict[str, Dict[str, float]]:
    ...
    for record in part_records:
        ...
        if part and city and rate is not None:
            rate_lookup.setdefault(city, {})[part] = rate
    return rate_lookup
...
def _build_part_pivot_map(records: List[Dict[str, str]]) -> Tuple[Dict[str, Dict[str, float]], List[str]]:
    ...
    for r in records:
        ...
        if part and city and val is not None:
            parts.setdefault(part, {})[city] = val
...
def _write_report_sheet(
    ...
    for record in core_rows:
        ...
        for col_idx, (_, key, parser) in enumerate(SUMMARY_EXPORT_FIELDS, start=1):
            ws.cell(row=row, column=col_idx, value=_coerce_value(record.get(key, ""), parser))

    row += 1
    ws.cell(row=row, column=2, value="月产成细项对比")
    ws.cell(row=row, column=8, value="月产值细项对比")
    ...
    for idx, city in enumerate(city_order):
        ws.cell(row=row, column=5 + idx, value=city)
    ws.cell(row=row, column=8, value="部位")
    for idx, city in enumerate(city_order):
        ws.cell(row=row, column=9 + idx, value=city)

    for part in PART_ORDER:
        row += 1
        ws.cell(row=row, column=2, value=part)
        auto_rate, nonauto_rate = _standard_rates_for_part(part)
        ws.cell(row=row, column=3, value=auto_rate if auto_rate is not None else "")
        ws.cell(row=row, column=4, value=nonauto_rate if nonauto_rate is not None else "")
        for idx, city in enumerate(city_order):
            val = part_map.get(part, {}).get(city)
            ws.cell(row=row, column=5 + idx, value=val if val is not None else "")
        value_label = part
        if part == "主产品":
            value_label = "主产品(整鸡还原)"
        elif part == "副产品":
            value_label = "副产品(整鸡还原)"
        ws.cell(row=row, column=8, value=value_label)
        for idx, city in enumerate(city_order):
            val = value_map.get(part, {}).get(city)
            ws.cell(row=row, column=9 + idx, value=val if val is not None else "")
