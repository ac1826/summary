...
def _canonical_header_name(name: str) -> str | None:
    normalized = _normalize_header_text(name)
    if normalized in {
        "\u4ea7\u6210\u7387(%)|\u603b\u8ba1",
        "\u4ea7\u6210\u7387|\u603b\u8ba1",
        "\u4ea7\u6210\u7387(%)|\u4e3b\u4ea7\u54c1",
...
def _header_priority(subheader_text: str) -> int:
    if subheader_text == "\u603b\u8ba1":
        return 3
    if subheader_text == "\u4e3b\u4ea7\u54c1":
        return 2
...
def _looks_like_core_subheader(row: List[str]) -> bool:
    labels = {_normalize_header_text(cell) for cell in row if _normalize_header_text(cell)}
    return bool(labels & {"\u4e3b\u4ea7\u54c1", "\u526f\u4ea7\u54c1", "\u603b\u8ba1"})


def build_column_map(header_row: List[str], subheader_row: List[str] | None = None) -> Dict[str, int]:
    mapping: Dict[str, int] = {}
    priorities: Dict[str, int] = {}
    last_header = ""
    for idx, name in enumerate(header_row):
        header_text = _normalize_header_text(name)
        subheader_text = ""
        if subheader_row is not None and idx < len(subheader_row):
            subheader_text = _normalize_header_text(subheader_row[idx])
        if header_text:
            last_header = header_text
        effective_header = header_text or (last_header if subheader_text else "")
        if not effective_header:
            continue
        candidates = []
        if subheader_text:
            candidates.append((f"{effective_header}|{subheader_text}", _header_priority(subheader_text)))
        candidates.append((effective_header, 1 if not subheader_text else _header_priority(subheader_text)))
        for candidate, priority in candidates:
            canonical = _canonical_header_name(candidate)
            if canonical and priority >= priorities.get(canonical, -1):
...
def extract_records_from_bytes(data: bytes, filename: str) -> List[Dict[str, str]]:
    ...
    has_core_subheader = bool(subheader_row and _looks_like_core_subheader(subheader_row))
    col_map = build_column_map(header_row, subheader_row if has_core_subheader else None)
    data_rows = rows[header_idx + 2 :] if has_core_subheader else rows[header_idx + 1 :]
...
def _recalc_sale_rate_with_external(records: List[Dict[str, str]]) -> None:
    """产销率统一按：销量(吨) / (生肉产量(吨) + 外采量(吨))。"""
    for record in records:
        sale = _parse_number(record.get("销量(吨)"))
        meat = _parse_number(record.get("生肉产量(吨)"))
        external = _parse_number(record.get("外采量(吨)")) or 0.0
        if sale is None or meat is None:
            continue
        denominator = meat + external
        if denominator <= 0:
            record["产销率(%)"] = ""
        else:
            # 保存为小数口径，后续 _format_percent / Excel 百分比格式会显示为百分数。
            record["产销率(%)"] = sale / denominator


def _should_force_part_percent_points(part_records: List[Dict[str, str]]) -> bool:
    """Detect whether part-rate raw numbers are stored as percent points (e.g. 24.7, 0.85)."""
    for record in part_records:
        text = str(record.get("产成率") or "").strip()
        ...
        try:
            ...
            continue
        # If any raw value is clearly larger than a fraction, treat this dataset as percent points.
        if abs(num) > 2:
            return True
    return False


def _parse_part_percent(val: str, force_percent_points: bool = False) -> float | None:
    """
    Parse part-rate values.
    - default: same behavior as _parse_percent (兼容历史文件)
    - force_percent_points=True: treat all numeric values as percent points from the sheet
      (e.g. 24.7 -> 24.7%, 0.85 -> 0.85%)
    """
    text = str(val or "").strip()
    if not text:
        return None
    ...
    try:
        ...
        return None
    if has_percent or force_percent_points:
        return num / 100
    if abs(num) > 2:
        return num / 100
    return num
...
def _build_rate_lookup(part_records: List[Dict[str, str]]) -> Dict[str, Dict[str, float]]:
    ...
    force_percent_points = _should_force_part_percent_points(part_records)
    for record in part_records:
        part = normalize_part_name(record.get("部位"))
        city = record.get("城市")
        rate = _parse_part_percent(record.get("产成率"), force_percent_points)
...
def _map_dataframe_values(df, func):
    if hasattr(df, "map"):
        return df.map(func)
...
def _build_part_pivot_map(records: List[Dict[str, str]]) -> Tuple[Dict[str, Dict[str, float]], List[str]]:
    if not records:
        return {}, []
    force_percent_points = _should_force_part_percent_points(records)
    if pd:
        dfp = pd.DataFrame(records)
        dfp["产成率"] = dfp["产成率"].apply(lambda v: _parse_part_percent(v, force_percent_points))
    ...
    for r in records:
        ...
        city = r.get("城市")
        val = _parse_part_percent(r.get("产成率"), force_percent_points)
...
def _build_value_pivot_map(
    ...
    if pd:
        ...
        explicit_unit_parts = set(DERIVED_PARTS) | {"副产品", "总计"}
        explicit_units = (
            dfv[dfv["部位"].isin(explicit_unit_parts)]
            .dropna(subset=["含税单价"])
            [["部位", "城市", "含税单价"]]
            .drop_duplicates(subset=["部位", "城市"], keep="first")
            .rename(columns={"含税单价": "_显式含税单价"})
        )
        if not explicit_units.empty:
            base = base.merge(explicit_units, on=["部位", "城市"], how="left")
            base["含税单价"] = base["_显式含税单价"].where(base["_显式含税单价"].notna(), base["含税单价"])
            base = base.drop(columns=["_显式含税单价"])
        ...
        for new_part, parts in DERIVED_PARTS.items():
            ...
            if new_part == "主产品" and rate_lookup:
                for city, part_rates in rate_lookup.items():
                    existing = base.loc[(base["部位"] == new_part) & (base["城市"] == city), "含税单价"]
                    if not existing.empty and pd.notna(existing.iloc[0]):
                        continue
                    numerator = 0.0
                    rate_sum = 0.0
        ...
        if "总计" in pivot.index and "主产品" in pivot.index:
            derived_sub = pivot.loc["总计"] - pivot.loc["主产品"]
            if "副产品" not in pivot.index:
                pivot.loc["副产品"] = derived_sub
            else:
                for city in pivot.columns:
                    if pd.isna(pivot.at["副产品", city]) and pd.notna(derived_sub.get(city)):
                        pivot.at["副产品", city] = derived_sub[city]
        pivot = pivot.reindex(PART_ORDER, fill_value=pd.NA)
        value_map = {}
        for part, row in pivot.iterrows():
    ...
    for new_part, comp in DERIVED_PARTS.items():
        comp_norm = _normalize_parts_list(comp)
        if new_part == "主产品" and rate_lookup:
            city_vals = dict(parts_map.get(new_part, {}))
            for city in all_cities:
                existing_unit = city_vals.get(city, {}).get("unit")
    ...
    if "总计" in parts_map and "主产品" in parts_map:
        sub_city = dict(parts_map.get("副产品", {}))
        for city in all_cities:
            if sub_city.get(city, {}).get("unit") is not None:
                continue
...
def main() -> None:
    ...
    _recalc_sale_rate_with_external(records)
    ...
    if pd:
        ...
        for col in ["生肉产量(吨)", "外采量(吨)", "销量(吨)", "宰鸡量(千只)"]:
        ...
        for r in core_view_sorted:
            ...
            item["外采量(吨)"] = _format_thousands0(r.get("外采量(吨)", ""))
    ...
    def _pivot_parts(records: List[Dict[str, str]]):
        ...
        if pd:
            dfp = pd.DataFrame(records)
            # 部位产成率表在部分文件中是“百分数点”原值（如 0.85 代表 0.85%）
            dfp["产成率"] = dfp["产成率"].apply(lambda v: _parse_part_percent(v, force_percent_points))
            dfp = dfp.dropna(subset=["产成率"])
            dfp["部位"] = dfp["部位"].apply(normalize_part_name)
            dfp = dfp.groupby(["部位", "城市"], as_index=False)["产成率"].sum()
    ...
    if pd and isinstance(pivoted, pd.DataFrame):
        ...
        part_csv = (
            _map_dataframe_values(pivoted, lambda x: f"{x*100:.2f}%" if pd.notna(x) else "").to_csv().encode("utf-8")
        )
    else:
    ...
    if pd and isinstance(pivot_v, pd.DataFrame):
        ...
        value_csv = (
            _map_dataframe_values(pivot_v, lambda x: _format_decimal2(x) if pd.notna(x) else "").to_csv().encode("utf-8")
        )
    else:
